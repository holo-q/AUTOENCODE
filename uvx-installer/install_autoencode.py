...
# The content of the autoencode.toml file
TOML_CONTENT = """# /autoencode command for gemini-cli
#
# Usage:
...
def main():
    """Main function to install the autoencode command."""
    command_dir = Path.home() / ".gemini" / "commands"
    command_file = command_dir / "autoencode.toml"

    print(f"Ensuring command directory exists at {command_dir}...")
    command_dir.mkdir(parents=True, exist_ok=True)

    print(f"Creating command file at {command_file}...")
    command_file.write_text(TOML_CONTENT)

    print("\n✅ The /autoencode command has been successfully installed for gemini-cli.")
    print("Restart your gemini-cli session to use the new command.")
